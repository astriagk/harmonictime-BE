# Test Accounts

Use these accounts for end-to-end testing across all roles. Real email accounts to monitor test flows.

**Shared Password:** `252318Gow@#`

---

## Admin

| Field    | Value              |
| -------- | ------------------ |
| Email    | astriagk@gmail.com |
| Password | 252318Gow@#        |
| Role     | admin              |

**Capabilities:** Full platform access — user management, seller approval, order oversight, catalog management, commission configuration, withdrawals.

---

## Sellers

**Capabilities:** List products, manage inventory, view orders, request withdrawals, manage bank account.

| #   | Email                  | Password    | Store Name         |
| --- | ---------------------- | ----------- | ------------------ |
| S1  | kronosqua@gmail.com    | 252318Gow@# | Chrono Vault       |
| S5  | gowtham0794k@gmail.com | 252318Gow@# | Gearworks Watch Co |

---

## Buyers

**Capabilities:** Browse catalog, place orders, make offers, track orders, manage wallet.

| #   | Email                 | Password    |
| --- | --------------------- | ----------- |
| B1  | thetime0794@gmail.com | 252318Gow@# |
| B2  | kronosqu2@gmail.com   | 252318Gow@# |

---

## Seller GST Details

| #   | GSTIN           | LegalBusinessName       | TradeName          | BusinessType | State       | PinCode |
| --- | --------------- | ----------------------- | ------------------ | ------------ | ----------- | ------- |
| S1  | 27AABFC1234A1Z5 | Chrono Vault Retail LLP | Chrono Vault       | LLP          | Maharashtra | 400001  |
| S5  | 24AACFG7890E1Z7 | Gearworks Watch Co LLP  | Gearworks Watch Co | LLP          | Gujarat     | 380001  |

> GSTINs above follow valid Indian format (state code + PAN pattern + entity + Z + check). Use these only for test environment — not real GSTINs.

---

## Notes

- All accounts use the shared password `252318Gow@#`
- Each email is unique — no overlap between admin, seller, and buyer accounts
- Register each email address in the system before assigning roles
- You'll receive confirmation and transactional emails at each address to monitor test flows
